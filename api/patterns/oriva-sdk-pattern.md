...
### 🔴 A byte-comparison guard is UNSATISFIABLE unless the generator AND formatter are pinned

This guard compares regenerated bytes against committed bytes. That only works if regeneration is
deterministic — and for three months it was not. `@hey-api/openapi-ts` was `^0.97.1` (0.99.0 was
published) and `prettier` was `^3.6.2` (3.9.6 was published), while this workflow installs with
`npm install`, not `npm ci`. So the output depended on **when** you generated it, and any
regeneration was near-certain to differ from whatever was committed.

Because the guard only runs when `packages/sdk/**` or the snapshot changes, an unsatisfiable
version of it can sit red for a long time — and the next person to touch the spec reasonably
concludes their own change broke it.

**Both are now pinned exactly** (`0.97.1`, `3.6.2`). If you ever loosen them back to a caret, this
guard becomes unsatisfiable again.

**Verify determinism, don't assume it** — run generate + workspace-prettier a SECOND time and
confirm it produces no further change. That idempotence check is the proof the guard can pass at
all; a single clean run only proves it agreed with itself once.

Note the workflow applies workspace Prettier AFTER `npm run generate`, because openapi-ts's
bundled Prettier does not read the workspace `.prettierrc` while husky's lint-staged does. Three
paths write this artefact — the generator, the pre-commit hook, and CI — and they must agree.
...
- `@oriva/mcp-server` depends on `@oriva/cli`, and `@oriva/cli` declares `@oriva/sdk` as a **peerDependency** — npm resolves both from the local workspace when running locally, and from the registry for a published consumer

🔴 **An internal range that cannot match the local version silently un-links the workspace.** When
`@oriva/cli` was bumped to 0.2.2 while `@oriva/mcp-server` still required `^0.1.0` (meaning
`>=0.1.0 <0.2.0`), npm could not satisfy it locally, fetched the **registry** copy of
`@oriva/cli@0.1.1`, and that published version dragged in an `@oriva/sdk` the lockfile had never
seen. `npm ci` then refuses outright with `Missing: @oriva/cli@0.1.1 from lock file`, which breaks
every CI job at the install step.

The quiet half is worse than the loud half: before `npm ci` gave up, builds were resolving
**published copies of our own packages** instead of the source sitting next to them.

Bumping any workspace package therefore means widening every range that points at it — including
`peerDependencies`, which is the one people miss. **Enforced**: `npm run check:workspace-versions`
(`scripts/check-workspace-versions.mjs`) asserts every internal range is satisfiable by the current
local version and that the lockfile records the same versions the manifests declare.
