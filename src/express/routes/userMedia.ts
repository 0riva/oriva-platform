 ...
 * User Media Routes
 *
 * Internal API endpoints for user media uploads (avatars, profile photos).
 * These routes only require user authentication, not API keys.
 * Routes: POST /user/media/avatar/upload-url, POST /user/media/avatar/confirm
 */
...
/**
 * POST /api/v1/user/media/avatar/upload-url
 * Generate pre-signed URL for avatar upload (internal use only)
 */
router.post(
  '/avatar/upload-url',
  requireJwtAuth,
...
/**
 * POST /api/v1/user/media/avatar/confirm
 * Confirm avatar upload and trigger AWS Rekognition validation
 */
router.post(
  '/avatar/confirm',
  requireJwtAuth,
  asyncHandler(async (req, res) => {
    ...
    const { key }: ConfirmUploadRequest = req.body;
    ...
    // Keys are formatted as: avatars/{userId}/{timestamp}-{uuid}.{ext}
    ...
    try {
      ...
      let inappropriateLabels: string[] = [];
      ...
      try {
        ...
        inappropriateLabels = moderationLabels.filter((label) => {
          const blockList = [
            'Explicit Nudity',
            'Nudity',
            'Sexual Activity',
        ...
        // If Rekognition fails (e.g., IAM permissions not configured), skip moderation
        console.warn(
          'Rekognition moderation skipped:',
          rekognitionError.code || rekognitionError.message
        );
      ...
      // Log if moderation was skipped
      if (moderationSkipped) {
        console.log('Avatar approved without moderation (Rekognition not available):', {
          key,
          photoId,
        });
      }
