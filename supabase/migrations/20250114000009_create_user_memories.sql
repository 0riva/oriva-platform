...
-- Constraints
ALTER TABLE user_memories ADD CONSTRAINT um_memory_type_check
  CHECK (memory_type IN ('conversation_context', 'user_preference', 'milestone', 'insight'));

ALTER TABLE user_memories ADD CONSTRAINT um_content_check
...
-- Comments
COMMENT ON TABLE user_memories IS 'User context and memory for personalized coaching';
COMMENT ON COLUMN user_memories.memory_type IS 'Type: conversation_context, user_preference, milestone, insight';
COMMENT ON COLUMN user_memories.importance IS 'Memory importance score (0.00-1.00)';
COMMENT ON COLUMN user_memories.relevance_decay_rate IS 'How fast importance decreases (0.00-1.00)';
