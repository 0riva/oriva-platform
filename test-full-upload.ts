import { createClient } from '@supabase/supabase-js';
import * as fs from 'fs';
import * as path from 'path';

const SUPABASE_URL = 'https://cbzgvlkizkdfjmbrosav.supabase.co';
const ANON_KEY =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNiemd2bGtpemtkZmptYnJvc2F2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTgwNDEwMjAsImV4cCI6MjA3MzYxNzAyMH0.GAqII3HLS7OKoKOWZxhfXhKEd55G1XpMPrM-MOJEdeg';
const BFF_URL = 'http://localhost:3002';

async function main() {
  // Sign in to get JWT token
  const supabase = createClient(SUPABASE_URL, ANON_KEY);

  console.log('=== Signing in ===');
  const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
    email: 'think@gavrielshaw.com',
    password: 'umn0xuh_CBP@jqc_exe',
  });

  if (authError) {
    console.error('Sign in error:', authError);
    return;
  }

  const token = authData.session?.access_token;
  console.log('Got token for user:', authData.user?.id);

  // Step 1: Get pre-signed upload URL
  console.log('\n=== Step 1: Getting pre-signed upload URL ===');
  const uploadResponse = await fetch(`${BFF_URL}/api/v1/user/media/avatar/upload-url`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
    },
    body: JSON.stringify({
      fileName: 'test-avatar.jpg',
      contentType: 'image/jpeg',
    }),
  });

  const uploadResult = (await uploadResponse.json()) as { uploadUrl?: string; key?: string };
  console.log('Upload URL received:', uploadResult.uploadUrl ? 'Yes' : 'No');
  console.log('File Key:', uploadResult.key);

  if (!uploadResult.uploadUrl) {
    console.error('Failed to get upload URL:', uploadResult);
    return;
  }

  // Step 2: Create a small test image (1x1 pixel red JPEG)
  console.log('\n=== Step 2: Creating test image ===');
  // This is a minimal valid JPEG file (1x1 red pixel)
  const minimalJpeg = Buffer.from([
    ,
  ]);
  console.log('Test image size:', minimalJpeg.length, 'bytes');

  // Step 3: Upload to S3
  console.log('\n=== Step 3: Uploading to S3 ===');
  const s3Response = await fetch(uploadResult.uploadUrl, {
    method: 'PUT',
    headers: {
      'Content-Type': 'image/jpeg',
    },
    body: minimalJpeg,
  });

  console.log('S3 Upload Status:', s3Response.status);

  if (s3Response.ok) {
    console.log('\n✅ SUCCESS! Image uploaded to S3');

    // Step 4: Confirm upload
    console.log('\n=== Step 4: Confirming upload ===');
    const confirmResponse = await fetch(`${BFF_URL}/api/v1/user/media/avatar/confirm`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({
        key: uploadResult.key,
      }),
    });

    console.log('Confirm Status:', confirmResponse.status);
    const confirmResult = (await confirmResponse.json()) as { url?: string };
    console.log('Confirm Response:', JSON.stringify(confirmResult, null, 2));

    if (confirmResult.url) {
      console.log('\n🎉 FULL FLOW COMPLETE!');
      console.log('Final Avatar URL:', confirmResult.url);
    }
  } else {
    const errorText = await s3Response.text();
    console.error('S3 Upload failed:', s3Response.status, errorText);
  }
}

main().catch(console.error);
